...
@task_router.get('/', summary='Lista das Tarefas', response_model=List[TaskDetail])
async def list_tasks(user: User = Depends(get_current_user)):
    return await TaskService.list_tasks(user)

@task_router.get('/create', summary='Detalhe de uma Tarefa por ID', response_model=TaskDetail)
